...
Fechas    <- seq(start(data.Set), by = "month", length.out = nrow(data.Set) + periodos.Forecast)
...
num.Filas    <- nrow(data.Set) + periodos.Forecast
...
pred.PMI <- forecast(PMI, h = periodos.Forecast)
...
pred.Bond10Y    <- forecast(Bond10Y, h = periodos.Forecast)
...
pred.Bond3M <- forecast(Bond3M, h = periodos.Forecast)
...
pred.UnEmpRate <- forecast(UnEmpRate, h = periodos.Forecast)
...
fila.proyeccion <- nrow(BBDD.Prob) - periodos.Forecast
